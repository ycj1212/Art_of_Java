...
// This class links keywords with their keyword tokens.
	...
	/*
	 * Table of keywords with their internal representation. All keywords must be
	 * entered lowercase.
	 */
	Keyword kwTable[] = { new Keyword("print", PRINT), // in this table.
			new Keyword("input", INPUT), new Keyword("if", IF), 
			new Keyword("then", THEN), new Keyword("goto", GOTO),
			new Keyword("for", FOR), new Keyword("next", NEXT), 
			new Keyword("to", TO), new Keyword("gosub", GOSUB),
...
// Execute an IF statement.
	private void execIf() throws InterpreterException {
		...
		/*
		 * If the result is true (non-zero), process target of IF. Otherwise move on to
...
// get the input var
		...
		try {
			...
			/*
			 * You might want to handle this error differently than the other interpreter
			 * errors.
			 */
